...
def decode_header(reader):
    typeByte = int(reader.read(1)[0])
    req_num = typeByte>>3
...
def decode_string(reader):
    req_num, type_num, length = decode_header(reader)
    raw_data = reader.read(length)
    return req_num, raw_data.decode()
...
def encode_tuple(code,val):
    tp = {int:0,tuple:2}[type(val)]
    header = bytes([(code<<3)+tp])
    b = encode(val)
...
def encode(s):
    ...
    for key,val in s.items():
        if val is None:
            ret_val += bytearray([(key<<3)+2,0])
        elif isinstance(val, int):
            b = encode_varint(val)
            ret_val += bytearray([(key<<3)]) + encode_varint(len(b)) + b
        elif isinstance(val, bytearray):
            b = val
            ret_val += bytearray([(key<<3)+2]) + encode_varint(len(b)) + b
